...
var ResolveFunc = func(host string) (ips []net.IP, e error) {
	return net.LookupIP(host)
}

// This file is copied from github.com/valyala/fasthttp/tcpdialer.go
// We need use self defined dns resolver to support non-system dns resolve
...
func getDialer(timeout time.Duration, dualStack bool) fasthttp.DialFunc {
	if timeout <= 0 {
...
var (
	...
	dialMap          = make(map[int]fasthttp.DialFunc)
	dialDualStackMap = make(map[int]fasthttp.DialFunc)
...
func (d *tcpDialer) NewDial(timeout time.Duration) fasthttp.DialFunc {
	d.once.Do(func() {
