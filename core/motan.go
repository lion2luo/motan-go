...
type RPCContext struct {
	...
	Meta *StringMap
	// SendTime represent the request(for client request) send time or response(for server response) send time
	SendTime time.Time
	// ReceiveTime represent the request(for server request) receive time or response(for client response) receive time
...
func (m *MotanRequest) Clone() interface{} {
	...
	if m.RPCContext != nil {
		newRequest.RPCContext = &RPCContext{
			...
			SendTime:       m.RPCContext.SendTime,
			ReceiveTime:    m.RPCContext.ReceiveTime,
