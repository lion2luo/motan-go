...
func (m *MotanEndpoint) checkChannelUsing() {
	...
	for {
		select {
		case <-checkerTimer.C:
			if time.Now().UnixNano()-m.getLastUsingTime() > int64(defaultChannelIdleInterval) {
				m.lock.Lock()
...
func (m *MotanEndpoint) Initialize() {
	...
	m.updateLastUsingTime(time.Now().UnixNano())
	isLazyInit := false
...
func (m *MotanEndpoint) updateLastUsingTime(timeNano int64) {
	atomic.StoreInt64(&m.lastChannelUsingTime, timeNano)
}

func (m *MotanEndpoint) getLastUsingTime() int64 {
	return atomic.LoadInt64(&m.lastChannelUsingTime)
}
...
func (m *MotanEndpoint) Call(request motan.Request) motan.Response {
	...
	m.updateLastUsingTime(startTime)
	if reqCtx.AsyncCall {
